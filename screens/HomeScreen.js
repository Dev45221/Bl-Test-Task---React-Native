...
import { View, Text, FlatList, Image, TouchableOpacity, Button, ToastAndroid } from "react-native"
import Ionicons from 'react-native-vector-icons/Ionicons';
import { useDispatch, useSelector } from "react-redux";
import { AddCart, RemoveCart, SignIn } from "../reduxCode/ActionCreator";
...
const HomeScreen = () => {
  ...
  const renderItem = ({ item }) => (
    <View style={{
      ...
      <Text style={{ width: 1, height: '100%', backgroundColor: 'grey' }} ></Text>
...
const CartScreen = () => {
  ...
  const RemoveItem = id => {
    // console.log(id)
    const res = cart.filter(item => item.id != id)
    // console.log(res);
  ...
  return (
    <View style={{ flex: 1 }} >
      <FlatList
        ...
        ListFooterComponent={() =>
          <View>
            <Text style={{ width: '100%', height: 1, backgroundColor: 'red', marginTop: 20 }} ></Text>
            <Text style={{ fontSize: 25, color: 'red', fontWeight: 'bold', letterSpacing: 2, fontFamily: 'georgia', margin: 10 }} >Total: ${cart.reduce((sum, item) => sum + item.price, 0)}</Text>
          </View>
        }
      />
    </View>
...
const ProfileScreen = () => {
  ...
  const getDetails = async () => {
    try {
      ...
      const { name, email, mobile, city, state } = data
      setName(name)
      setEmail(email)
      setMobile(mobile)
      setCity(city)
